contract MyTokenShr {
	...
	modifier onlyActive {if(active) _; } // ORIGINAL: modifier onlyActive {if(active) _ }
    modifier onlyfromcompany {if(msg.sender == address(myCompany)) _; } // ORIGINAL: modifier onlyfromcompany {if(msg.sender == address(myCompany)) _ }
...
//	uint256 percOfBuffer=100;
	...
	function closeBooks() {
		...
		bool res = address(Company(CompanyAddress)).send(inc); // ORIGINAL: bool res = Company(CompanyAddress).send(inc);
	}
	function dumpOut() {
		if(msg.sender != CompanyAddress) throw; //Only Internal Call..	
...
contract Board is Globals{
    ...
    function _isBoardMember(address c) returns(bool){
	    ...
	    modifier onlybyboardmember {if(_isBoardMember(tx.origin)) _; } // ORIGINAL: modifier onlybyboardmember {if(_isBoardMember(tx.origin)) _ }
...
contract SlotMachineMngr is Board{	//
	...
	function _slotCommitNew(address _addr) public onlybyboardmember {
		...
		Slot.setCompany();
		bool res = address(Slot).send(startSlotAt); // ORIGINAL: bool res = Slot.send(startSlotAt);
		Slots.push(Slot);	
		addSlotBy = 0;		
	}
...
contract ProjectMngr is Board {
	...
	function _projectCommitNew(address _addr) public onlybyboardmember {
		...
		bool res = address(myProject).send(newProjectBudget); // ORIGINAL: bool res = myProject.send(newProjectBudget);
